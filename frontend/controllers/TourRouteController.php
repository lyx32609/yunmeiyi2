...
//     	$res->RuleDetail='00';//数组需要处理成字符串
    	...
    	if($isPost){	$posts = \Yii::$app->request->post();  
	    	if ($tourRoute->load($posts,'') && $tourRoute->validate($posts,'') && $tourRoute->save()) {//验证保存
	    		echo 11;die;
	    	}echo 22;die;
    	}echo 33;die;
    }
	...
	public function actionAdd(){
		...
		if($posts){
			...
			&& $tourroutemodel->save() && $routedaymodel->load($posts,'') && //旅游线路日程安排
			$routedaymodel->validate($posts,'') && $routedaymodel->save() && 
			$routedailymodel->load($posts,'') && $routedailymodel->validate($posts,'') //旅游线路每天线路详情 
			...
			$tourroutemodel->RouteNo=$posts;
			$tourroutemodel->RouteTypeID=$posts;
			$tourroutemodel->Days=$posts;
			$tourroutemodel->Nights=$posts;
			$tourroutemodel->OrganizationID=$posts;
			$tourroutemodel->StartCityID=$posts;
			$tourroutemodel->DestinationCityID=$posts;
			$tourroutemodel->EndCityID=$posts;
			$tourroutemodel->MinPassengerCount=$posts;
			$tourroutemodel->MaxPassengerCount=$posts;
			$tourroutemodel->AheadDays=$posts;
			$tourroutemodel->Period=$posts;
			$tourroutemodel->AdultPrice=$posts;
			$tourroutemodel->ChildPrice=$posts;
			$tourroutemodel->SingleRoomPrice=$posts;
			$tourroutemodel->validate();
			$tourroutemodel->save(); */
			if ($tourroutemodel->load($posts,'') && $tourroutemodel->validate($posts,'') 
				&& $tourroutemodel->save()) {
				//添加成功
